...
Add a soft, saturated glow around images, SVGs, videos, and other content.
...
```bash
npx shadcn@latest add https://ui.iandefined.com/r/backlight.json
```
...
The component keeps the filter definition hidden and applies it directly to a single native element or to an internal wrapper around grouped content. Images, SVGs, and canvases can use the SVG filter directly. For video, mirror each frame to a canvas so WebKit can render the filtered glow while the native video remains visible and interactive. Cross-origin iframes can block SVG filter input, so use a same-page glow source behind an embedded player. Set `blur` to control the glow intensity.
...
export default function BacklightSvgDemo() {
  return (
    <div className="flex w-full items-center justify-center px-6 py-10">
      <Backlight className="rounded-full">
        <svg
        ...
        >
          <g fill="#58C4DC" fillRule="evenodd">
...
Mirror a same-origin `<video>` to a canvas behind it when the glow should follow the video frames. The example uses `requestVideoFrameCallback` with a `requestAnimationFrame` fallback, so the backlight works in Safari and other modern browsers without filtering the native video layer.
...
| :--- | :--- | :--- | :--- |
| `blur` | `number` | `10` | Sets the `stdDeviation` value used by the SVG Gaussian blur filter. Higher values create a wider glow. |
