...
Submit without accepting a listed tag to mark the autocomplete invalid. Query text and accepted values are validated separately, so correction clears the error only after a listed option is chosen.
