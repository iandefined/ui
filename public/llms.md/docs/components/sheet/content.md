...
Compare default and inset footers with long content across every floating sheet edge.
...
export default function SheetFloatingFooterDemo() {
  return (
    <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {footerVariants.map(({ side, footerVariant }) => {
        const sideLabel = capitalize(side);
        ...
        return (
          <Sheet key={`${side}-${footerVariant}`}>
            ...
            >
              <SheetHeader>
                <SheetTitle>{sideLabel} floating sheet</SheetTitle>
                <SheetDescription>
                  The body scrolls while the complete bordered surface and its
                  footer stay together.
                </SheetDescription>
              </SheetHeader>
              <SheetBody fadeEdges="y">
                <div className="grid gap-3">
