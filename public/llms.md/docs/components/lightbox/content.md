...
| `peek` | `boolean` | `false` | Shows part of the previous and next slides beside the active slide. |
| `navigation` | `boolean` | `true` | Groups discovered images into a navigable collection. Set it to `false` to hide the counter, carousel, and previous and next controls, and mount only the selected item. |
| `loop` | `boolean` | `true` | Wraps navigation from the last item to the first item and back. |
| `preload` | `number` | `1` | Number of adjacent items to render and preload on each side. |
