...
```bash
npx shadcn@latest add https://ui.iandefined.com/r/marquee.json
```
...
| :--- | :--- | :--- | :--- |
| `speed` | `number` | `30` | Duration of one complete loop in seconds. Higher values move slower. |
| `direction` | `"left" \| "right"` | `left` | Direction in which the content travels. |
| `pauseOnHover` | `boolean` | `false` | Pauses the loop while the pointer is over the marquee. |
| `gap` | `number \| string` | `2rem` | Gap between items and repeated content groups. Numbers are interpreted as pixels; strings can be any CSS length. |
| `fade` | `boolean` | `true` | Adds a soft fade at both edges of the viewport while content is moving. |
| `fadeColor` | `string` | `-` | Sets the CSS color used by the edge fades. It accepts any CSS color value, including a theme variable such as `var(--card)` or `var(--popover)`. It is optional because the default mask automatically blends into the surface behind the marquee. The fade is hidden when the marquee is not moving. |
| `onlyOnOverflow` | `boolean` | `true` | Starts the animation only when the natural content width is wider than the available viewport. |
