...
If solidifying an icon or opening an inverse seam leaves a corner sharper than the
canonical Lucide geometry, round it. Preserve the source curve or stroke join with a
rounded join, arc, or equivalent filled contour; do not leave pointed tips, mitered
corners, or sharp seam-to-edge intersections when the source reads as rounded. Check
these transitions at 16px and 24px, especially where an open seam pierces a silhouette.
