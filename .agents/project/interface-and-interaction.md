...
- Structural regions inside a translated overlay, including headers and footers, must inherit the overlay's movement by default. Counter-transforms that pin a region to the viewport must be opt-in through an explicit prop and limited to motion that collapses the surface toward that region; never let the region detach from the closing surface. When bordered surfaces collide, overlap their borders by one pixel instead of rendering a two-pixel seam.
- Floating drawers with snap points must keep the primitive's measured popup shell dimensionally stable. Anchor a separate inner bordered surface to the viewport inset and resize it by reserving a transparent block-start inset inside the fixed border-box; do not translate the visible surface or derive the measured popup's dimensions from observed descendant geometry. Keep structural regions inside that rounded, overflow-clipped inner surface so footers cannot detach or overflow its perimeter.
- Base UI registers `--drawer-snap-point-offset` and `--drawer-swipe-movement-*` with `inherits: false` to limit high-frequency style recalculation. Consume them on `DrawerPopup` whenever possible. Expose inheriting aliases only for descendant behavior that requires them, and do not propagate those aliases through a floating snap drawer's deep content tree. Disable inset transitions during initial/ending styles and direct swipes so opening stays fast and dragging remains synchronous.
- A full-height measurement shell must not become an invisible interaction layer. Keep drawer viewports and floating snap shells pointer-transparent, restore pointer events only on the visible surface, and let the modal backdrop receive empty-area wheel and outside-press events without allowing interaction with the underlying page. Enter and exit transforms must travel only the visible snapped height, not the full measurement-shell height. Keep a nonzero backdrop blur/opacity floor at partial snap points, and render popup focus outlines on the visible surface rather than the transparent measurement shell.
- For expensive portalled drawers, retain only the targeted drawer after its first opening transition completes, and keep its closed DOM invisible, inert, and laid out. Do not eagerly mount every drawer on a page or mount a full popup subtree from trigger intent. Opening must use snap geometry derived synchronously from the controlled or uncontrolled snap value so its first animation frame does not wait for primitive measurement callbacks.
- Do not move full drawer portal mounting or forced hidden layout into `pointerdown`; it can block the activation that is supposed to begin the animation. Prefer lazy first mount, then opt into retained layout only after the first opening transition completes. This keeps first-open backdrop and surface motion in the same paint opportunity while preserving fast repeat opens.
- For floating snap drawers, keep the last non-null visible snap offset stable throughout dismissal. The popup owns direct swipe translation and enter/exit translation; the fixed measurement shell's block-start inset owns snap resizing. Direct swipes must not mutate that inset, and starting/ending styles must exclude it from the transition-property list so a close cannot expand the surface before translating it away.
- Use Base UI's `data-swipe-dismiss` state for velocity-scaled release timing and clamp the computed duration to a finite interaction range. Do not key release duration from `data-swiping`, which is cleared as the pointer gesture hands off to the closing transition.
