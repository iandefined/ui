...
- For expensive portalled drawers, preload only the targeted drawer on trigger intent (`pointerenter`, `pointerdown`, or focus), retain it after first use, and keep its closed DOM invisible, inert, and laid out. Do not eagerly mount every drawer on a page. Opening must use snap geometry derived synchronously from the controlled or uncontrolled snap value so its first animation frame does not wait for primitive measurement callbacks.
