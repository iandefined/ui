...
- Structural regions inside a translated overlay, including headers and footers, must inherit the overlay's movement by default. Counter-transforms that pin a region to the viewport must be opt-in through an explicit prop; when bordered surfaces collide, overlap their borders by one pixel instead of rendering a two-pixel seam.
