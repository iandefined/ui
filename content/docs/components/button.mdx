---
title: Button
description: A button with oklch-tinted hover and active states, custom colors, variants, sections, and sizes.
---

Use `Button` for actions that submit, confirm, navigate, or change application state.

## Preview

<ComponentPreview name="button/default" />

## Installation

<ComponentInstall name="button" />

## Usage

```tsx
import { Button } from "@/components/ui/button";

<Button>Get started</Button>;
```

The default variant uses the oklch tint treatment. Pass a CSS color to `color` to set its background color; hover and active states mix the current foreground color into that base.

```tsx
<Button className="text-white" color="#2563eb">
  Get started
</Button>
```

## Examples

### Variants

Choose a visual treatment for the action.

<ComponentPreview name="button/variants" />

### Custom Colors

Apply `color` to the default variant.

<ComponentPreview name="button/custom-colors" />

### Sizes

Match the button to its surrounding controls.

<ComponentPreview name="button/sizes" />

### With Icons

Use `leftSection` and `rightSection` to place icons beside the label.

<ComponentPreview name="button/with-icons" />

### Loading

Disable the action while work is in progress.

<ComponentPreview name="button/loading" />

### Disabled

Show an unavailable action without changing its layout.

<ComponentPreview name="button/disabled" />

### As Link

Use Base UI's `render` prop and set `nativeButton={false}` when rendering a link.

<ComponentPreview name="button/as-link" />

## Accessibility

Give icon-only buttons an `aria-label`. Use `nativeButton={false}` only when `render` supplies a real non-button target.

## API Reference

`Button` wraps [Base UI Button](https://base-ui.com/react/components/button). Supported Base UI props pass through.

### Props

<ApiPropsList>
  <ApiProp
    name="variant"
    fullType='"default" | "secondary" | "outline" | "ghost" | "link" | "destructive"'
    simpleType="string"
    defaultValue='"default"'
  >
    Sets the visual treatment. `color` is available only when this is
    `"default"` or omitted.
  </ApiProp>
  <ApiProp name="color" fullType="string">
    Sets the default background color used for the oklch hover and active tint.
  </ApiProp>
  <ApiProp
    name="size"
    fullType='"xs" | "sm" | "default" | "lg" | "xl" | "icon-xs" | "icon-sm" | "icon" | "icon-lg" | "icon-xl"'
    simpleType="string"
    defaultValue='"default"'
  >
    Sets the button dimensions.
  </ApiProp>
  <ApiProp
    name="radius"
    fullType='"none" | "sm" | "default" | "lg" | "xl" | "full"'
    simpleType="string"
    defaultValue='"default"'
  >
    Sets the corner radius.
  </ApiProp>
  <ApiProp name="leftSection" fullType="ReactNode">
    Renders content before the label.
  </ApiProp>
  <ApiProp name="rightSection" fullType="ReactNode">
    Renders content after the label.
  </ApiProp>
</ApiPropsList>
