---
title: Badge
description: Compact labels for status, category, or metadata.
---

Use `Badge` to communicate concise status, category, or count information.

## Preview

<ComponentPreview name="badge/default" />

## Installation

<ComponentInstall name="badge" />

## Usage

```tsx
import { Badge } from "@/components/ui/badge";

<Badge>Badge</Badge>;
```

## Features

Set `color` with `translucent` to choose a palette-driven appearance. Semantic variants such as `success` and `warning` own their color.

## Examples

### Semantic variants

Choose a semantic state.

<ComponentPreview name="badge/variants" />

### Palette colors

Use a palette color with a palette-driven variant.

<ComponentPreview name="badge/colors" />

### Sizes

Use compact badges in dense layouts.

<ComponentPreview name="badge/sizes" />

### Depth

Use `depth="surface"` to add an inset highlight shadow and subtle border. Badges default to `depth="flat"`.

<ComponentPreview name="badge/depth" />

### With Icon

Add a small, meaningful icon.

<ComponentPreview name="badge/with-icon" />

### With Link

Render a navigation target through `render`.

<ComponentPreview name="badge/with-link" />

### With Count

Communicate a compact count.

<ComponentPreview name="badge/with-count" />

## API Reference

`Badge` uses Base UI's [`useRender`](https://base-ui.com/react/utils/use-render) for polymorphic rendering. Standard element props pass through.

### Props

<ApiPropsList>
  <ApiProp
    name="variant"
    fullType='"default" | "secondary" | "outline" | "destructive" | "error" | "info" | "success" | "warning" | "translucent"'
    simpleType="string"
    defaultValue='"default"'
  >
    Sets the semantic or palette-driven appearance.
  </ApiProp>
  <ApiProp
    name="depth"
    fullType='"flat" | "surface"'
    simpleType="string"
    defaultValue='"flat"'
  >
    Sets the surface depth treatment. Use `"surface"` for an inset highlight shadow and subtle border.
  </ApiProp>
  <ApiProp
    name="color"
    fullType="BadgeColor"
    simpleType="string"
    defaultValue='"gray"'
  >
    Sets a palette color when `variant` is `"translucent"`.
  </ApiProp>
  <ApiProp
    name="size"
    fullType='"default" | "compact"'
    simpleType="string"
    defaultValue='"default"'
  >
    Sets the badge height and spacing.
  </ApiProp>
</ApiPropsList>
