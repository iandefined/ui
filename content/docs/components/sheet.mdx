...
<ComponentPreview name="sheet/floating-footer" />
...
When an application layout triggers this artifact, use the `data-overlay="blur"` attribute on `[data-slot="sheet-backdrop"]` to disable the backdrop filter and apply an ordinary `filter: blur(...)` directly to the children of the application scroll container. Keep the sheet portal mounted outside the blurred container.

```css title="app.css"
@property --app-sheet-blur {
  syntax: "<length>";
  inherits: false;
  initial-value: 0px;
}

[data-slot="sheet-backdrop"][data-overlay="blur"]:not([hidden]) {
...
:root {
  --app-sheet-blur: 0px;
  transition: --app-sheet-blur 200ms cubic-bezier(0.32, 0.72, 0, 1);
}

:root:has([data-slot="sheet-backdrop"][data-overlay="blur"]:not([hidden]):not([data-ending-style])) {
  --app-sheet-blur: 8px;
}

.app-scroll-container > * {
  filter: blur(var(--app-sheet-blur));
}
