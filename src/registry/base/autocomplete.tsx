  ...
  const popupContent = (
    <BaseAutocomplete.Popup
      className={cn(
        ...
        "data-[side=top]:not-has-data-[slot=autocomplete-input]:translate-y-(--safe-offset) data-[side=top]:not-has-data-[slot=autocomplete-input]:pb-[calc(var(--anchor-height)+4px)]",
        className
      )}
      data-slot="autocomplete-popup"
      {...props}
    >
      <span className="pointer-events-auto flex min-w-0 max-h-[min(var(--available-height),20rem)] w-(--anchor-width) max-w-(--available-width) flex-col overflow-hidden not-has-data-[slot=autocomplete-input]:w-[calc(var(--anchor-width)+10px)] not-has-data-[slot=autocomplete-input]:max-w-[calc(var(--available-width)+10px)]">
        {children}
      </span>
    </BaseAutocomplete.Popup>
  );
